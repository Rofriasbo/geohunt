...
class _WelcomeScreenState extends State<WelcomeScreen> {
  ...
  void _initFCMListeners() {
    // 1. Manejar el mensaje cuando la app está en PRIMER PLANO
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      print("FCM Message (Foreground) received: ${message.data}");

      // Puedes usar aquí la notificación local que ya tienes para avisar
      if (message.notification != null) {
        mostrarNotificacion(
          message.notification!.title ?? 'Alerta GeoHunt',
          message.notification!.body ?? 'Hay una novedad cerca.',
          'tesoro',
        );
      }
    });

    // 2. Manejar la interacción (clic) con la notificación cuando la app está en SEGUNDO PLANO
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
      print('FCM Message opened app: ${message.data}');
      // Lógica de navegación si el usuario hace clic en la notificación
    });

    // 3. Manejar notificaciones cuando la app está TERMINADA
    // Necesitas una función de nivel superior o estática para esto.
    FirebaseMessaging.instance.getInitialMessage().then((RemoteMessage? message) {
      if (message != null) {
        print('FCM Message terminated app: ${message.data}');
        // Lógica de navegación al abrir la app desde la notificación
      }
    });
  }
...
class _UserMapViewState extends State<UserMapView> {
  ...
  void _startListeningLocation() {
    const LocationSettings locationSettings = LocationSettings(
        ...
        distanceFilter: 2 // Esto asegura que la posición se actualice con frecuencia
    );
        ...
        children: [
          ...
          StreamBuilder<DocumentSnapshot>(
            ...
            builder: (context, userSnapshot) {
              ...
              // Creamos un UserModel FRESC O a partir del snapshot
              final userData = userSnapshot.data!.data() as Map<String,
                  dynamic>;
              final currentUser = UserModel.fromMap(userData, widget.user.uid);
              ...
              return StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, treasureSnapshot) {
                  ...
                  // TreasureModel? closestTreasure; // No se usa

                  if (treasureSnapshot.hasData) {
                    final allDocs = treasureSnapshot.data!.docs;
                    final allTreasures = allDocs.map((d) =>
                        TreasureModel.fromMap(d.data() as Map<String, dynamic>,
                            d.id)).toList();

                    // Lista solo de los NO encontrados. ¡Usa el usuario FRESCO!
                    final uncollectedTreasures = allTreasures
                        .where((t) =>
                    !(currentUser.foundTreasures?.contains(t.id) ?? false))
                        .toList();

                    TreasureModel? foundTreasureInRange;

                    // Lógica de Proximidad (Sólo para no encontrados)
                    if (_currentPosition != null) {
                      for (var t in uncollectedTreasures) {
                        final double dist = _distanceCalculator.as(
                            LengthUnit.Meter, _currentPosition!,
                            LatLng(t.location.latitude, t.location.longitude));

                        if (dist <= 5) {
                          foundTreasureInRange = t;
                          break;
                        }
                      }

                      if (foundTreasureInRange?.id != _treasureInRange
                          ?.id) { // Comparamos por ID para ser seguro
                        // Usamos microtask para actualizar el estado después del build
                        Future.microtask(() {
                          setState(() {
                            _treasureInRange = foundTreasureInRange;
                    ...
                    // Construcción de Marcadores (TODOS LOS TESOROS)
                    markers = allTreasures.map((t) {
                      // ¡Usa el usuario FRESCO para la lógica de tesoros encontrados!
                      bool isFound = currentUser.foundTreasures?.contains(
                          t.id) ?? false;
                      Color markerColor;
                      ...
                      if (isFound) {
                        ...
                        markerColor =
                            Colors.green; // 🟢 VERDE: ¡LISTO PARA CAZAR!
                      } else {
                        markerColor = Colors.red; // 🔴 ROJO: Activo pero lejos
                      }
                    ...
                    // Ruta solo hacia tesoros pendientes
                    if (_showRoute && _currentPosition != null) {
                      routePoints =
                          _calculateOptimizedRoute(uncollectedTreasures);
                    }
                  }

                  // Si el tesoro está siendo eliminado y no se encontró data, sigue mostrando el indicador
                  if (!treasureSnapshot.hasData &&
                      treasureSnapshot.connectionState !=
                          ConnectionState.waiting) {
                    return const Center(
                        child: Text('No hay tesoros disponibles.'));
                  }

                  if (_currentPosition != null) {
                    markers.add(Marker(point: _currentPosition!,
                        width: 50,
                        height: 50,
                        child: const Icon(
                            Icons.person_pin_circle, color: Colors.blue,
                            size: 40)));
                  }
                  ...
                  return FlutterMap(
                    ...
                    options: MapOptions(
                        initialCenter: _tepicCenter, initialZoom: 14),
                    children: [
                      TileLayer(
                          urlTemplate: 'https://tile.openstreetmap.org/{z}/{x}/{y}.png',
                          userAgentPackageName: 'com.geohunt.app'),
                      if (_showRoute && routePoints.isNotEmpty) PolylineLayer(
                      ...
                      if (_treasureInRange != null && _currentPosition != null)
                        CircleLayer(circles: [
                          CircleMarker(
                              point: LatLng(_treasureInRange!.location.latitude,
